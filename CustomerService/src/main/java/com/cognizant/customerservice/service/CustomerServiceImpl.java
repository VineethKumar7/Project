...
public class CustomerServiceImpl implements CustomerService {
	...
	public CustomerEntity createCustomer(String token, CustomerEntity customer) {
		...
		if (checkCustomerExists != null) {
			throw new ConsumerAlreadyExistException("Customer already exists");
	...
	public Loan checkLoan(String token, String id,long amount) {
		CustomerEntity customerEntity=getCustomerDetail(token,  id);
		System.out.println("HIdtfgyhujkjihuygtfrde"+customerEntity);
		List<Account> list=customerEntity.getAccounts();
		Loan loan=new Loan();
		...
		loan.setIschecked(false);
		loanRepository.save(loan);
		//System.out.println("HIdtfgyhujkjihuygtfrde"+loan);
		
		return loan;
	}
public List<Loan> ischecked(){
	return loanRepository.findByIschecked(false);
}
	@Override
